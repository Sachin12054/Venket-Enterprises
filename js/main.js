...
class VenkatEnterprisesApp {
    ...
    // Payment history and confirmPayment removed
    ...
    createProductCard(product) {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                ...
                ${stockStatus}
                <!-- Cart and wishlist buttons removed -->
            </div>
        `;
        return card;
    }
    ...
    renderProducts(clearContainer = true) {
        ...
        // Show only 6 bestsellers (featured) on home
        let productsToShow = this.products.filter(p => p.featured);
        if (productsToShow.length < 6) {
            // Fill up to 6 with other products if not enough featured
            const ids = new Set(productsToShow.map(p => p.id));
            productsToShow = productsToShow.concat(
                this.products.filter(p => !ids.has(p.id)).slice(0, 6 - productsToShow.length)
            );
        }
    ...
    showProductModal(product) {
        try {
            ...
            modalOverlay.addEventListener('click', (e) => {
                ...
                // Cart and wishlist modal actions removed
            });
    ...
    injectStyles() {
        if (document.getElementById('venkat-app-styles')) return;
        ...
        style.textContent = `
            ...
            .dark-theme .product-specifications li {
